...
The 📷 button next to every barcode field (`VesselBarcodeField` and the Vessel Lookup search bar) opens [`BarcodeScanner`](src/components/barcode-scanner.tsx) — a camera overlay that decodes a QR/barcode from the live video feed via `@zxing/browser`'s `BrowserMultiFormatReader`, requesting the rear (`facingMode: "environment"`) camera on phones/tablets. It's a single-shot scan: first successful decode stops the camera and fills the field, and the camera stream is always torn down on close/unmount (cleanup stops both the decode `controls` and every track on the `MediaStream` we hold — leaving a camera running is the classic bug here). `NotAllowedError` (permission denied) and "no camera found" get distinct, visible error states rather than an infinite spinner.

**We manage `getUserMedia`/`video.play()` ourselves rather than delegating to `@zxing/browser`'s `decodeFromConstraints`.** Their internal `play()` call happens a few async hops away from the button tap that opened the modal, and on browsers with a stricter autoplay policy that gets silently blocked — their own code (`BrowserCodeReader.tryPlayVideo`) catches the rejection with only a `console.warn`, so the failure never reaches app code. Symptom, reproduced during testing: a permanently black video box, camera light on, no error shown. Fix: request the stream and call `.play()` ourselves so we can catch that rejection, then fall back to a "Tap to start camera" overlay — a fresh, direct user gesture that satisfies the policy — before handing the now-playing `<video>` element to `decodeFromVideoElement` for the actual decode loop.
